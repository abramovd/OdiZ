...
from Rsum import Rsum
from convsumR import convsumR

def Zsum(Z1A, Z1As, Z1B, Z1Bs, Z2A, Z2As, Z2B, Z2Bs):
    ...
    Z3A, Z3As = Rsum(Z1A,Z1As,Z2A,Z2As)
    u = p1
    v = p2
    ZconvM = []
    for k1 in range(n1):
        for k2 in range(n2):
            ZconvM.append(convsumR(u[:, k1], v[:, k2]))
    ...
    for k1 in range(n1):
        for k2 in range(n2):
            Z12Bs[k1, k2] = np.dot(Z3A, convsumR(u[:, k1], v[:, k2]))
...
if __name__ == "__main__":
    ...
    print(Zsum(Z1A, Z1As, Z1B, Z1Bs, Z2A, Z2As, Z2B, Z2Bs))