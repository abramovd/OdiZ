...
def Rdiv(A, As, B, Bs):
    ...
    for i in range(m1):
        for j in range(m2):
            if Z1As[i] == 0 and Z2As[j] == 0:
                supp32[i, j] = float('NaN')
            elif Z2As[j] == 0:
                supp32[i, j] = float('Inf')
            else:
                supp32[i, j] = Z1As[i] / Z2As[j]
    ...
    for k in range(1, m3 + 1):
        for i in range(1, m1 + 1):
            for j in range(1, m2 + 1):
                if (i / j >= (2 * k - 1) / m3) and (i / j <= (2 * k + 1) / m3):
...
if __name__ == "__main__":
    print(Rdiv([1, 2, 3, 4], [0.2, 0.4, 0.1, 0.3], [5, 2, 3, 6], [0.1, 0.3, 0.5, 0.8]))